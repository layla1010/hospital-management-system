...
public class AddMedicationToTreatment extends JPanel {

    private static final long serialVersionUID = 1L;
    private CardLayout cardLayout;
    private JPanel cardsPanel;

    public AddMedicationToTreatment(Treatments d, Treatment curTreatment) {
        ...
        // Title Label
        JLabel lblTitle = new JLabel("Add Medication to Treatment " + curTreatment.getSerialNumber());
        ...
        JComboBox<Medication> comboBox = new JComboBox<>();
        comboBox.setBackground(new Color(0x698DB0));
        
        comboBox.setModel(new DefaultComboBoxModel<>((Medication[]) Hospital.getInstance().getMedications().values().toArray(new Medication[0])));
        comboBox.setRenderer(new ListCellRenderer<Medication>() {
        ...
        btnUpdate.addActionListener(e -> {
            try {
                Medication selectedMedication = (Medication) comboBox.getSelectedItem();
                curTreatment.addMedication(selectedMedication);
                d.refreshList();
                
                JOptionPane.showMessageDialog(null, "Added Medication to Treatment successfully!");
